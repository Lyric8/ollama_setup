...
Mac 作为服务端运行时，系统休眠会导致 Ollama 停止响应。用 `caffeinate` 阻止休眠：
...
更彻底的方案：系统设置 → 节能 → 将「防止自动进入睡眠」打开。

---

### 新 Terminal 窗口找不到 ollama 命令

刚安装 Homebrew 后，已有的 Terminal 窗口不会自动更新 PATH。有两种解法：

```bash
# 临时生效（当前窗口）
eval "$(/opt/homebrew/bin/brew shellenv zsh)"

# 永久生效：关闭当前 Terminal，重新打开
```
...
网络不稳定时，用以下脚本自动断点续传，断了自动重试直到完成：

```bash
#!/bin/bash
MODEL="gemma4:e4b"   # 改成目标模型名
...
while true; do
    ...
    echo "Not complete, retrying in 3s..."
    sleep 3
done
```

> ⚠️ **下载期间不要切换网络。** Ollama 通过文件 hash 识别续传位置，换网络后 CDN 节点可能返回不同 hash，导致已下载内容作废，从头重新下载。

---

### KEEP_ALIVE=-1 的陷阱

将 `OLLAMA_KEEP_ALIVE` 设为 `-1` 会让模型永久驻留内存，**但同时会阻止模型切换**。

原因：26b 占用约 20GB 显存，e4b 需要约 11GB，两者无法同时加载。当 26b 永久驻留时，发给 e4b 的请求会卡住等待内存释放，而内存永远不会释放。

**推荐方案：** 保持 `KEEP_ALIVE=10m`，Ollama 会在内存不足时自动卸载旧模型、加载新模型，切换完全自动，只需要等待模型加载时间（e4b 约 5 秒，26b 约 20 秒）。

---

### 流式输出

实际使用中推荐开启流式输出，可以边生成边读取，降低感知延迟：
...
resp = requests.post("http://SERVER_IP:11434/api/generate", json={
    "model": "gemma4:e4b",
    "prompt": "你好",
    "stream": True,
}, stream=True)
...
`OLLAMA_NUM_PARALLEL=2` 是全局设置，对所有模型生效：

- **e4b + 并发 2**：总显存约 13GB，M1 Pro 32GB 完全可行
- **26b + 并发 2**：总显存超过 21GB 上限，Ollama 会自动降到并发 1，无需手动处理，但不要期望 26b 真的并发
...
# 查看错误日志
tail -50 ~/OllamaModels/ollama-error.log
...
curl http://localhost:11434/api/version   # 本地能通
curl http://192.168.x.x:11434/api/version  # 局域网不通？

# 检查 macOS 防火墙
# 系统设置 → 网络 → 防火墙 → 确认 ollama 被允许接受传入连接
```

### 模型切换卡住

原因：当前模型设置了 `KEEP_ALIVE=-1`，永久占用显存。
...
或者将 plist 中的 `OLLAMA_KEEP_ALIVE` 改回 `10m` 并重启服务。

### 请求超时 / 响应很慢

- 检查是否有其他应用占用大量内存（Activity Monitor → 内存）
- 26b 首次加载需要 20-30 秒，属正常现象
- 推理模式（`think: true`）生成 token 更多，耗时成倍增加，按需使用
